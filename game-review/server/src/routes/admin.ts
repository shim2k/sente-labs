  ...
  try {
    ...
    try {
      const analytics: any = {
        ...
        engagementSegments: [],
        tokenEconomy: {},
        profileLinkingAnalysis: [],
        profileLinkingSummary: []
      };

      // 1. User Token Usage Analysis
      const userTokenUsageQuery = `
      ...
      `;
      analytics.userTokenUsage = (await client.query(userTokenUsageQuery)).rows;

      // 2. Overall Platform Statistics
      const platformStatsQuery = `
        ...
        LEFT JOIN review_tasks rt ON g.db_id = rt.game_db_id AND rt.job_state = 'completed'
      `;
      analytics.platformStats = (await client.query(platformStatsQuery)).rows[0];
      ...
      `;
      analytics.tokenDistribution = (await client.query(tokenDistributionQuery)).rows;

      // 4. Users at Risk (Zero Tokens)
      ...
      `;
      analytics.usersAtRisk = (await client.query(usersAtRiskQuery)).rows;

      // 5. Game Upload and Review Activity
      const gameActivityQuery = `
        SELECT 
          COALESCE(i.aoe4world_username, i.username, u.email, 'Anonymous') as user_display,
          COUNT(DISTINCT g.id) as games_uploaded,
          COUNT(DISTINCT r.id) as reviews_created,
          ...
          MIN(g.played_at) as first_game_upload,
          MAX(g.played_at) as last_game_upload,
      ...
      `;
      analytics.gameActivity = (await client.query(gameActivityQuery)).rows;

      // 6. Review Model Preferences
      const modelPreferencesQuery = `
        SELECT 
          rt.llm_model,
      ...
      `;
      analytics.modelPreferences = (await client.query(modelPreferencesQuery)).rows;

      // 7. Daily Activity Trends (Last 30 Days)
      const dailyTrendsQuery = `
        ...
        LIMIT 30
      `;
      analytics.dailyTrends = (await client.query(dailyTrendsQuery)).rows;

      // 8. Popular Game Modes and Maps
      const popularGameModesQuery = `
        SELECT 
          g.game_mode,
          g.team_size,
        ...
        LIMIT 10
      `;
      analytics.popularGameModes = (await client.query(popularGameModesQuery)).rows;

      const popularMapsQuery = `
        SELECT 
          g.map_name,
          COUNT(DISTINCT g.id) as games_count,
          COUNT(DISTINCT r.id) as reviews_count,
        ...
        LIMIT 10
      `;
      analytics.popularMaps = (await client.query(popularMapsQuery)).rows;

      // 9. User Engagement Segments
      const engagementSegmentsQuery = `
        ...
        ORDER BY 
          ...
          END
      `;
      analytics.engagementSegments = (await client.query(engagementSegmentsQuery)).rows;

      // 10. Token Economy Health
      const tokenEconomyQuery = `
        WITH user_tokens AS (
          SELECT 
            COUNT(DISTINCT u.id) as total_users,
        ...
        review_tasks_stats AS (
          ...
          FROM review_tasks rt
          WHERE rt.job_state = 'completed'
        )
        SELECT 
          ut.total_users,
          ut.total_tokens_in_circulation,
          ut.total_tokens_consumed,
          (ut.total_tokens_in_circulation + ut.total_tokens_consumed) as total_tokens_issued,
          ROUND(ut.depleted_users * 100.0 / NULLIF(ut.total_users, 0), 2) as user_depletion_rate_percentage,
          rts.total_review_tasks,
          rts.standard_review_tokens_spent,
          rts.elite_review_tokens_spent,
          (rts.standard_review_tokens_spent + rts.elite_review_tokens_spent) as calculated_tokens_spent,
          ROUND((rts.standard_review_tokens_spent + rts.elite_review_tokens_spent) * 100.0 / 
                NULLIF(ut.total_tokens_consumed, 0), 2) as token_calculation_accuracy_percentage
        FROM user_tokens ut
        CROSS JOIN review_tasks_stats rts
      `;
      analytics.tokenEconomy = (await client.query(tokenEconomyQuery)).rows[0];

      // 11. Profile Linking Analysis
      const profileLinkingAnalysisQuery = `
      ...
      `;
      analytics.profileLinkingAnalysis = (await client.query(profileLinkingAnalysisQuery)).rows;

      // 12. Profile Linking Summary
      const profileLinkingSummaryQuery = `
        ...
        FROM profile_summary
      `;
      analytics.profileLinkingSummary = (await client.query(profileLinkingSummaryQuery)).rows;
